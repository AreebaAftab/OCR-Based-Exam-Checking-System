...
# app.config['UPLOADS_DEFAULT_DEST']='static/teacher'
# app.config['UPLOADED_PHOTOS_DEST']='static/students'
...
# 	if request.method=='POST' and 'mcqstd_file' in request.files:
# 		# myfile = request.files['photo']
# 		# filename=photos.save(myfile, 'img', str(myfile)+'.txt')
# 		mstdfilename=photos.save(request.files['mcqstd_file'])
		
# 	return render_template('check2.html',mcqpaper=mcqpaper,mstdfilename=mstdfilename)

# @app.route('/uploadteacher',methods=['POST'])
# def uploadteacher():
# 	global mteafilename
# 	if request.method=='POST' and 'mcqtea_file' in request.files:
# 		# myfile = request.files['photo']
# 		# filename=photos.save(myfile, 'img', str(myfile)+'.txt')
# 		mteafilename=photos.save(request.files['mcqtea_file'])
		
# 	return render_template('check2.html',mcqpaper=mcqpaper,mstdfilename=mstdfilename,mteafilename=mteafilename)
